...
const fetchSalesforceObjects = async (instanceUrl: string, accessToken: string) => {
    try {
        const response = await axios.get(`${instanceUrl}/services/data/v54.0/sobjects`, {
            headers: {
...
const fetchObjectFields = async (instanceUrl: string, accessToken: string, objectName: string) => {
    try {
...
const fetchChildRelationships = async (
    ...
    try {
        ...
        console.error(`Error fetching child relationships for object ${objectName}:`, error);
        throw error;
    }
};
...
const fetchRelatedFiles = async (
    ...
    try {
        ...
        const templateQueryUrl = `${instanceUrl}/services/data/v54.0/query?q=${encodeURIComponent(templateQuery)}`;
        ...
        const templateIds = templateResponse.data.records.map((record: any) => record.Id);
        ...
        const fileQueryUrl = `${instanceUrl}/services/data/v54.0/query?q=${encodeURIComponent(fileQuery)}`;
...
export { fetchSalesforceObjects, fetchObjectFields, fetchChildRelationships, fetchRelatedFiles, fetchAndConvertFileToHtml };
